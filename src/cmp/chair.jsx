...
const Chair = () => {
  ...
  const [ frame ] = useTrimesh(() => ({ type: "Static", args: frameArgs, position: [ -5, 0, 0 ]}), useRef())
  const [ basket ] = useTrimesh(() => ({ mass: 10, args: basketArgs, position: [ -5, 2.5, 0 ]}), useRef())
  const [ chainA ] = useBox(() => ({ mass: 2, args: [ 0.1, 0.42, 0.1 ], position: [ -5, 4.8, 0 ]}), useRef())
  const [ chainB ] = useBox(() => ({ mass: 2, args: [ 0.1, 0.42, 0.1 ], position: [ -5, 4.7, 0 ]}), useRef())
